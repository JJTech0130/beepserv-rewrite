    ...
    - (void) readLogsWithCompletion:(void (^)(BOOL))completion {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            NSString* logFileContent = [[NSString
                stringWithContentsOfFile: kLogFilePath encoding: NSUTF8StringEncoding error:&readingErr]
                stringByTrimmingCharactersInSet: [NSCharacterSet newlineCharacterSet]
            ];

            if (readingErr) {
                lastLogFileContent = [NSString stringWithFormat:@"Couldn't read log file: %@", readingErr];
